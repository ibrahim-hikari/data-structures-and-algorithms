...
const filterStringsWithVowels = (arr) => {
    return arr.filter(word => /[aeiou]/.test(word));  //found it in the internet I didn't get it yet
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
    let result = arr.filter(snor => snor.baseStat > minBaseStat);
    return result;
};
...
const getStatName = (arr, minBaseStat) => {
    let result = getBaseStatGreaterThan(arr, minBaseStat).map(snor => snor.stat.name);
    return result
};
